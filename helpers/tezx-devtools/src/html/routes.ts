...
type RouteMap = Record<
  string, // pattern
  {
    [middleware: string]: any[]; // method (used as middleware name)
  }
>;
...
export function Routes(ctx: Context, app: TezX) {
  ...
  }[] = Object.entries(routeMap).map(([pattern, middlewareMap]) => ({
    pattern,
    handlers: Object.entries(middlewareMap).map(([method, handlers]) => ({
      method,
      handlerNames: handlers.map((fn: any) => fn.name || "[anonymous]"),
    })),
  }));

  const rawJSON = JSON.stringify(allRoutes, null, 2);

  const toCSV = (data: typeof allRoutes) => {
    const headers = ["Path", "Middleware", "Handler"];
    const rows = data.flatMap((route) =>
      route.handlers.flatMap((mw) =>
        mw.handlerNames.map((h) => [
  ...
  <div class="tabs toolbar">
    ...
    <h2 class="router-title">📌 Router: <span class="router-name">${(app as any).router.name}</span></h2>
  <div id="searchBar">
